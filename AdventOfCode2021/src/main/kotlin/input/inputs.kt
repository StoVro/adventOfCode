...
val INPUT_DAY10_CHUNKS : Array<String>
    get() = arrayOf("<[{([[<{{(<{[({}<>){{}{}}][<<>{}>[<>{}]]}(<<<>()}{[]()}>([{}()][()()]))>[({({}[])(<>())}){[({}()){[][]}]{[",
            "{{{{(([((<[{<<(){}>{<>}>{<(){}>{()[]}}}([{()]<{}{}>])]{<{{()[]}{[]{}}}(<[]<>>{()[]})>}>[[<{([]{}){",
            "[([(({<{<<<[((<><>){<>[]})]<[<[]()>[[][]]]({<>[]}<()<>>)>><<({<><>}[()[]])[[<>{}]<[][]>]>>",
            "<<<[{({[{([{{{()()}<<><>>}[{[]()}]}[({()<>}{()[]}){<{}<>>({}[])}]]({{{[]<>}<<>{}]}}<<([]{}){()[]}><({}",
...
val INPUT_DAY11_ENERGIES : MutableList<MutableList<Int>>
    get() = mutableListOf(mutableListOf(6,7,4,4,6,3,8,4,5,5),
            mutableListOf(3,1,3,5,7,4,5,4,1,8),
            mutableListOf(4,7,5,4,1,2,3,2,7,1),
            mutableListOf(4,2,2,4,2,5,7,1,6,1),
            mutableListOf(8,1,6,7,1,8,6,5,4,6),
            mutableListOf(2,2,6,8,5,7,7,6,7,4),
            mutableListOf(7,1,7,7,7,6,8,1,7,5),
